...
<body style="width:100%">
	...
	<div class="span12">
		<br><legend style="text-align:center">FRIENDS LIST</legend><br><br>
		<ul class='thumbnails'>
		   <?php 
		   		require_once('connect.php');
		   		$id = $_SESSION['id'];
				$sql = "select user.PPID, user.First, user.ID from user where ID in (select `friend list`.UID from `friend list` where `friend list`.FR_ID = '$id') or ID in(select `friend list`.FR_ID from `friend list` where `friend list`.UID = '$id')"; 
        		...
        		<div class="thumbnail clearfix" style="text-align:center;height:150px">
            		...
            		<form method="post" action="frndpro.php" style="display:inline">
								...
								<button type="submit" class="btn-link">'.$row['First'].'</button>
					</form>
            	</div>
    		</li> ';
							}}}?>
