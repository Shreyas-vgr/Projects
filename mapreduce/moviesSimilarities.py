...
class MoviesSimilarities(MRJob):
    ...
    def count_ratings_users_freq(self, user_id, values):
        ...
        (item,rating pairs)
        Also emit  count and user rating sum for use later steps.
    ...
    def calculate_ranking(self, item_keys, values):
        ...
        '''
        corr_sim, cos_sim, reg_corr_sim, n = values
        item_x, item_y = item_keys
        if int(n) > 0:
            yield (item_x, corr_sim, cos_sim, reg_corr_sim), \
                     (item_y, n)
    ...
    def top_similar_items(self, key_sim, similar_ns):
        ...
        '''
        item_x, corr_sim, cos_sim, reg_corr_sim = key_sim
        for item_y, n in similar_ns:
            yield None, (item_x, item_y, corr_sim, cos_sim, reg_corr_sim, n)
